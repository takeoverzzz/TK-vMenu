...
namespace vMenuClient
{
    ...
    public class CustomCam : BaseScript
    {
        ...
        private void UpdateParams()
        {
            ...
            if (LeadCam)
            {
                EnhancedCamera.driftCamera = EnhancedCamera.CreateNonAttachedCamera();
                EnhancedCamera.driftCamera.IsActive = true;
            }
            else if (ChaseCam)
            {
                EnhancedCamera.chaseCamera = EnhancedCamera.CreateNonAttachedCamera();
                EnhancedCamera.chaseCamera.IsActive = true;
            }
            // Update GUI params
        ...
        private void CreateMenu()
        {
            ...
            presetsMenu.OnItemSelect += (sender, item, index) => {
                ...
                {
                    EnhancedCamera.Notify("~g~~h~Info~h~~s~: Switching to FPV camera base. Tune XYZ offsets to your car.");
                ...
                {
                    EnhancedCamera.Notify("~g~~h~Info~h~~s~: Switching to NFS camera. Tune XYZ offsets to your car.");
            ...
            savedCamerasMenu.OnItemSelect += (sender, item, index) => {
                if (item == saveCamera)
                {
                    ...
                    {
                        EnhancedCamera.Notify("~g~~h~Info~h~~s~: You are currently not in any vehicle. Please enter a vehicle before trying to save the camera.");
                    }
                }
                else
                {
            ...
            selectedCameraMenu.OnItemSelect += async (sender, item, index) => {
                ...
                {
                    string newName = await EnhancedCamera.GetUserInput(windowTitle: "Enter a new name for this camera.", defaultText: null, maxInputLength: 30);
                    ...
                    else
                    {
                        if (SaveCameraInfo("xcm_" + newName, currentlySelectedCamera.Value, false))
                        {
                            ...
                            EnhancedCamera.Notify("~g~~h~Info~h~~s~: Your camera has successfully been renamed.");
                            selectedCameraMenu.GoBack();
            ...
            menu.OnListIndexChange += (_menu, _listItem, _oldIndex, _newIndex, _itemIndex) => {
                ...
                if (_listItem == customCamFOVList)
                {
                    ...
                    if (LeadCam)
                    {
                        SetCamFov(EnhancedCamera.driftCamera.Handle, fov);
                    }
                    else if (ChaseCam)
                    {
                        SetCamFov(EnhancedCamera.chaseCamera.Handle, fov);
                    }
                }
        ...
        private async Task RunDriftCam()
        {
            if (LeadCam)
            {
                ...
                if (vehicleEntity > 0)
                {
                    if (EnhancedCamera.driftCamera != null)
                    {
                        ...
                        if (!linearPosOffset)
                        {
                            if (oldPosXOffset != finalRotation)
                            {
                                ...
                                if (pedLock)
                                {
                                    EnhancedCamera.driftCamera.Position = veh.Position + EnhancedCamera.CamMath.RotateAroundAxis(staticPosition, Vector3.ForwardLH, rotation * EnhancedCamera.CamMath.DegToRad);
                                }
                                else
                                {
                                    EnhancedCamera.driftCamera.Position = veh.Position + EnhancedCamera.CamMath.RotateAroundAxis(staticPosition, veh.UpVector, rotation * EnhancedCamera.CamMath.DegToRad);
                                }
                                if (EnhancedCamera.userLookBehind)
                                {
                                    EnhancedCamera.driftCamera.Position = veh.Position +
                            ...
                            else
                            {
                                ...
                                {
                                    EnhancedCamera.driftCamera.Position = veh.Position + staticPosition;
                                }
                            }
                        }
                        else
                        {
                            EnhancedCamera.driftCamera.Position = veh.Position + staticPosition + veh.RightVector * oldPosXOffset / 12f;
                            if (EnhancedCamera.userLookBehind)
                            {
                                EnhancedCamera.driftCamera.Position = veh.Position + EnhancedCamera.CamMath.RotateAroundAxis(staticPosition, veh.UpVector, 179f * EnhancedCamera.CamMath.DegToRad);
                            }
                        }
                        ...
                        if (CameraConstraints.CrashCheck(vehicleEntity))
                        {
                            ...
                            EnhancedCamera.driftCamera.Position = veh.Position + staticPosition;
                            roll = EnhancedCamera.CamMath.Lerp(EnhancedCamera.driftCamera.Rotation.Y, 0f, 0.1f);
                            pitch = EnhancedCamera.CamMath.Lerp(EnhancedCamera.driftCamera.Rotation.X, 0f, 0.1f);
                            pitch = CameraConstraints.ClampPitch(pitch);
                        }
                        else
                        {
                            // Calculate smooth roll and pitch rotation
                            roll = EnhancedCamera.CamMath.Lerp(EnhancedCamera.driftCamera.Rotation.Y, -GetEntityRoll(vehicleEntity), cameraRollInterpolation);
                            pitch = EnhancedCamera.CamMath.Lerp(EnhancedCamera.driftCamera.Rotation.X - EnhancedCamera.userTilt, GetEntityRotation(vehicleEntity, 2).X, cameraPitchInterpolation);
                        ...
                        SetCamRot(EnhancedCamera.driftCamera.Handle, pitch + EnhancedCamera.userTilt, roll, yaw, 2);
                ...
                else
                {
                    ...
                    if (LeadCam || ChaseCam)
                    {
                        EnhancedCamera.SwitchToGameplayCam();
                        EnhancedCamera.Notify("~g~~h~Info~h~~s~: Vehicle not found, switching to gameplay camera...");
                    }
                }
            }
            else
        ...
        private async Task RunChaseCam()
        {
            if (ChaseCam)
            {
                ...
                if (vehicleEntity > 0)
                {
                    if (EnhancedCamera.chaseCamera != null)
                    {
                        ...
                        if (target != null)
                        {
                            ...
                            {
                                target = null;
                                EnhancedCamera.SwitchCameraToDrift();
                                EnhancedCamera.Notify("~g~~h~Info~h~~s~: Target exceeded angle limit, switching to Lead Camera");
                                return;
                            }
                            ...
                            if (finalRotation.ToString() != "NaN")
                            {
                                ...
                                finalRotation = EnhancedCamera.CamMath.Lerp(GetEntityHeading(EnhancedCamera.chaseCamera.Handle), finalRotation, 1 - angCamInterpolation);
                                ...
                                if (!lockOffsetPos)
                                {
                                    ...
                                    if (pedLock)
                                    {
                                        EnhancedCamera.chaseCamera.Position = veh.Position + EnhancedCamera.CamMath.RotateAroundAxis(staticPosition, Vector3.ForwardLH, rotation * EnhancedCamera.CamMath.DegToRad);
                                    }
                                    else
                                    {
                                        EnhancedCamera.chaseCamera.Position = veh.Position + EnhancedCamera.CamMath.RotateAroundAxis(staticPosition, veh.UpVector, rotation * EnhancedCamera.CamMath.DegToRad);
                                    }
                                    if (EnhancedCamera.userLookBehind) { EnhancedCamera.chaseCamera.Position = veh.Position - (veh.RightVector * sideOffset) + EnhancedCamera.CamMath.RotateAroundAxis(staticPosition, veh.UpVector, 179f * EnhancedCamera.CamMath.DegToRad); }
                                }
                                else
                                {
                                    EnhancedCamera.chaseCamera.Position = veh.Position + staticPosition;
                                    if (EnhancedCamera.userLookBehind) { EnhancedCamera.chaseCamera.Position = veh.Position + staticPosition + veh.ForwardVector * 3f + veh.UpVector * 0.5f; }
                                }
                                ...
                                if (CameraConstraints.CrashCheck(vehicleEntity))
                                {
                                    staticPosition = Vector3.ForwardLH * upOffset;
                                    EnhancedCamera.chaseCamera.Position = veh.Position + staticPosition;
                                    roll = EnhancedCamera.CamMath.Lerp(EnhancedCamera.chaseCamera.Rotation.Y, 0f, 0.1f);
                                    pitch = EnhancedCamera.CamMath.Lerp(EnhancedCamera.chaseCamera.Rotation.X, 0f, 0.1f);
                                    pitch = CameraConstraints.ClampPitch(pitch);
                                }
                                else
                                {
                                    // Calculate smooth roll and pitch rotation
                                    roll = EnhancedCamera.CamMath.Lerp(EnhancedCamera.chaseCamera.Rotation.Y, -GetEntityRoll(vehicleEntity), cameraRollInterpolation);
                                    pitch = EnhancedCamera.CamMath.Lerp(EnhancedCamera.chaseCamera.Rotation.X - EnhancedCamera.userTilt, GetEntityPitch(vehicleEntity), cameraPitchInterpolation);
                                    roll = CameraConstraints.ClampRoll(roll);
                                    pitch = CameraConstraints.ClampPitch(pitch);
                                }
                                // Finally, set the rotation
                                float yaw = (EnhancedCamera.userLookBehind) ? (GetEntityRotation(vehicleEntity, 2).Z + 179.9f) : (newRot.Z + EnhancedCamera.userYaw);
                                EnhancedCamera.chaseCamera.Rotation = new Vector3(pitch + EnhancedCamera.userTilt, roll, yaw);
                        ...
                        else
                        {
                            ...
                            EnhancedCamera.SwitchCameraToDrift();
                            EnhancedCamera.Notify("~g~~h~Info~h~~s~: Target not found, switching to Lead Camera");
                        }
        ...
        public async void SaveCamera(string updateExistingSavedCameraName = null)
        {
            ...
            if (Game.PlayerPed.IsInVehicle())
            {
                ...
                if (updateExistingSavedCameraName == null)
                {
                    ...
                    if (!string.IsNullOrEmpty(saveName))
                    {
                        ...
                        {
                            EnhancedCamera.Notify($"~g~~h~Info~h~~s~: Camera {saveName} saved.");
                            LoadCameras();
                        }
            ...
            {
                EnhancedCamera.Notify("~g~~h~Error~h~~s~: You need to be inside the vehicle");
            }
        }
